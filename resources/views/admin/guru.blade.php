...
@section('konten')
<form class="card" method="post" action="{{ route('admin.guru.simpan') }}">
    @csrf
    ...
    <p>
        <label>
            <input type="checkbox" name="role[]" value="pengampu mata pelajaran" style="width:auto">
            Pengampu Mata Pelajaran
        </label>

        <label>
            <input type="checkbox" name="role[]" value="wali kelas" style="width:auto">
            Wali Kelas
        </label>

        <label>
            <input type="checkbox" name="role[]" value="staff" style="width:auto">
            Staff
        </label>
    </p>
...
<table>
    ...
    @foreach($guru as $item)
        <tr>
            ...
            <td>{{ isset($roles[$item->id]) ? $roles[$item->id]->pluck('role')->join(', ') : '-' }}</td>
            <td>{{ $item->telepon }}</td>
            <td>
