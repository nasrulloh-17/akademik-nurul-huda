...
@section('body')
<style>
    .nav{position:fixed;z-index:5;left:0;right:0;top:0;padding:18px 7vw;display:flex;justify-content:space-between;align-items:center;background:rgb(0, 71, 76);backdrop-filter:blur(12px);border-bottom:1px solid var(--garis)}
    .hero{padding:110px 7vw 50px;background:linear-gradient(135deg,#007979,#24B1B1);min-height:620px}.hero-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:28px;align-items:center}.hero h1{font-size:clamp(34px,5vw,62px);line-height:1.05;margin:0;color:var(--putih)}.slider{height:390px;border-radius:8px;overflow:hidden;box-shadow:0 26px 70px rgba(4,120,105,.18);background:linear-gradient(135deg,var(--toska),var(--hijau));position:relative}.slider-track{display:flex;height:100%;transition:transform .55s ease}.slide{min-width:100%;height:100%;display:grid;place-items:end start;padding:34px;color:white;background-size:cover;background-position:center;position:relative}.slide:before{content:"";position:absolute;inset:0;background:linear-gradient(0deg,rgba(0,0,0,.52),rgba(0,0,0,.05))}.slide div{position:relative;max-width:85%}.slider-btn{position:absolute;top:50%;z-index:2;transform:translateY(-50%);width:42px;height:42px;border:0;border-radius:999px;background:rgba(255,255,255,.9);color:var(--hijau);font-size:24px;font-weight:800;cursor:pointer;box-shadow:0 10px 30px rgba(0,0,0,.16)}.slider-btn:hover{background:white}.slider-btn.prev{left:14px}.slider-btn.next{right:14px}.slider-dots{position:absolute;left:0;right:0;bottom:16px;z-index:2;display:flex;justify-content:center;gap:8px}.slider-dot{width:10px;height:10px;border:0;border-radius:999px;background:rgba(255,255,255,.52);cursor:pointer}.slider-dot.active{width:26px;background:white}.section{padding:54px 7vw}.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:18px}.news-img{width:100%;height:180px;object-fit:cover;border-radius:8px;background:var(--muda)}.login-menu{position:relative}.login-menu:hover .drop{display:block}.drop{display:none;position:absolute;right:0;top:42px;background:white;border:1px solid var(--garis);border-radius:8px;box-shadow:0 18px 45px rgba(0,0,0,.12);min-width:170px}.drop a{display:block;padding:12px 14px}
...
<section class="hero">
    <div class="hero-grid">
        <div>
            ...
            <p style="font-size:19px;color:#ffffff;max-width:620px">Sistem informasi akademik terpadu.</p>
...
<section class="section">
    <h2>Kontak</h2>
    <div class="card"><p>Munjuk, Labuhan Maringgai</p><p>Email: admin@ppnurulhuda.or.id</p></div>
</section>
<script>
