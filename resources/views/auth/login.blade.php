...
<div style="min-height:100vh;display:grid;place-items:center;background:linear-gradient(135deg,#e8fffb,#f7fffd)">
    <form method="post" class="card" style="width:min(430px,92vw)" action="{{ $jenis === 'admin' ? route('admin.login.proses') : ($jenis === 'guru' ? route('guru.login.proses') : route('siswa.login.proses')) }}">
        @csrf
